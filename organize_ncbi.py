...
def main():
    ...
    for assembly_name in df.accession.values:
        ...
        # Chage permission so anyone in the group can modify

        for dirpath, dirnames, filenames in os.walk(f'{args.target}/{species_name}'):
            os.chmod(dirpath, 0o775)
            for filename in filenames:
                filepath = os.path.join(dirpath, filename)
                os.chmod(filepath, 0o664)  # Set file permissions

        # os.chmod(f'{args.target}/{species_name}', 0o775)
